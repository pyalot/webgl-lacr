exports = class Grid
    @title = '1.7 Derivative Maps and Lighting'
    @cameraPos = [0, 2, 4]
    ...
    constructor: (@app) ->
        ...
        @aspectSelect = app.addSelect
            label: 'Aspect'
            value: 2
            options: [
                {name:'Derivatives', option:0}
                {name:'Normals', option:1}
                {name:'Lit', option:2}
            ]
            onValue: (value) =>
                console.log value
                @aspectNum = parseInt(value, 10)
